...
export default function PreventiveView({
  ...
  // Helper to get Argentina / Cordoba exact time
  ...
  const [currentTime, setCurrentTime] = useState<Date>(getArgentinaDate());

  React.useEffect(() => {
    const timer = setInterval(() => {
      setCurrentTime(getArgentinaDate());
    }, 1000);
    return () => clearInterval(timer);
  }, []);

  const currentYear = currentTime.getFullYear();
  const currentMonth = currentTime.getMonth(); // 0-indexed
  const currentDay = currentTime.getDate(); // 1-indexed
  ...
  // Calendar parameters for dynamic current month
  // Get first day of current month in Argentina timezone
  const firstDayOfMonth = new Date(currentYear, currentMonth, 1);
  const dayOfWeek = firstDayOfMonth.getDay(); // 0 is Sunday, 1 is Monday, etc.
  const emptyDaysOffset = dayOfWeek === 0 ? 6 : dayOfWeek - 1; // Week starts on Monday
  const totalDaysInMonth = new Date(currentYear, currentMonth + 1, 0).getDate();
  const calendarGridCells = Array.from({ length: emptyDaysOffset + totalDaysInMonth });

  // Get task for a specific day in the current dynamic month/year
  const getTasksForDay = (day: number) => {
    const dateString = `${currentYear}-${(currentMonth + 1).toString().padStart(2, '0')}-${day.toString().padStart(2, '0')}`;
    return tasks.filter(t => {
      if (t.date === dateString) return true;
      // If task has 2026-10-xx hardcoded date, show it on the current month's corresponding day
      const dayPart = t.date.split('-')[2];
      if (t.date.startsWith('2026-10-') && parseInt(dayPart, 10) === day) {
        return true;
      }
      return false;
    });
    ...
    <div className="space-y-6" id="preventive-tab-content">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white border border-outline-variant rounded-xl overflow-hidden shadow-sm flex flex-col" id="calendar-bento-card">
          <div className="p-4 border-b border-outline-variant flex flex-col sm:flex-row sm:items-center justify-between bg-surface-container gap-2">
            <div className="flex items-center gap-2 text-primary">
              <Calendar size={18} />
              <span className="text-sm font-bold">Calendario de {monthName} {currentYear}</span>
            </div>
            {/* Live Córdoba time indicator */}
            <div className="text-xs bg-white text-emerald-800 border border-emerald-100 rounded-full px-3 py-1 font-mono flex items-center gap-1.5 shadow-2xs">
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
              <span>Córdoba, AR: {currentTime.toLocaleTimeString('es-AR', { hour: '2-digit', minute: '2-digit', second: '2-digit' })}</span>
            </div>
          </div>
          ...
          <div className="bg-white border border-outline-variant rounded-xl p-6 shadow-sm flex-1 flex flex-col justify-between">
            <div>
              <h3 className="text-xs font-extrabold text-on-surface-variant uppercase tracking-wider mb-3">
                Tareas del Día: {selectedDay ? `${monthName} ${selectedDay}` : 'Ninguno seleccionado'}
              </h3>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            >
              ...
              <form onSubmit={handleCreateTask} className="p-6 space-y-4">
                ...
                {/* Fecha */}
                <div className="space-y-1">
                  <label className="text-xs font-bold text-on-surface-variant uppercase tracking-wider">Fecha programada ({monthName} {currentYear})</label>
                  <input 
                    type="date"
                    required
                    value={newDate}
