...
export default function App() {
  ...
  useEffect(() => {
    const initAndSubscribe = async () => {
      ...
      const unsubIncidents = subscribeIncidents((loaded) => {
        setIncidents(loaded);
      });

      const unsubTasks = subscribePreventiveTasks((loaded) => {
        setPreventiveTasks(loaded);
      });

      const unsubActivities = subscribeActivities((loaded) => {
        const sorted = [...loaded].sort((a, b) => b.id.localeCompare(a.id));
        setActivities(sorted);
      });

      const unsubApproved = subscribeApprovedWorkers((loaded) => {
        setApprovedWorkers(loaded);
      });

      const unsubPending = subscribePendingWorkers((loaded) => {
  ...
  const handleAddActivityLog = async (title: string, description: string, statusText: string) => {
  ...
  const handleApproveWorker = async (workerEmail: string) => {
    ...
    await deletePendingWorker(workerEmail);
    await saveApprovedWorker(approvedWorker);
  ...
  const handleRejectWorker = async (workerEmail: string) => {
  ...
  const handleUpdateWorker = async (email: string, updatedData: Partial<UserProfile>) => {
    ...
    if (foundApproved) {
      const updated = { ...foundApproved, ...updatedData };
      await saveApprovedWorker(updated);
      if (userProfile && userProfile.email.toLowerCase() === email.toLowerCase()) {
        setUserProfile(updated);
      }
    } else if (foundPending) {
      const updated = { ...foundPending, ...updatedData };
      await savePendingWorker(updated);
      if (userProfile && userProfile.email.toLowerCase() === email.toLowerCase()) {
        setUserProfile(updated);
      }
    }
  ...
  const handleDeleteWorker = async (email: string) => {
    ...
    if (foundApproved) {
      await deleteApprovedWorker(email);
    }
    if (foundPending) {
      await deletePendingWorker(email);
    }
  ...
  if (!userProfile) {
    return (
      <GateScreen 
        ...
        onRegisterWorker={async (newWorker) => {
          await savePendingWorker(newWorker);
  ...
  const handleUpdateIncidentStatus = async (id: string, newStatus: StatusType, cost?: number, actionsTaken?: string) => {
  ...
  const handleUpdateIncidentPriority = async (id: string, newPriority: PriorityType) => {
    const inc = incidents.find(i => i.id === id);
    if (!inc) return;
    const updated = { ...inc, priority: newPriority };
  ...
  const handleCompleteTask = async (id: string) => {
    const task = preventiveTasks.find(t => t.id === id);
    if (!task) return;
    ...
    await saveActivity(taskAct);

    const updatedTask = { ...task, status: 'Completada' as const };
    await savePreventiveTask(updatedTask);
  };

  // Callback: Schedule new preventive task
  const handleAddPreventiveTask = async (newTaskData: Omit<PreventiveTask, 'id'>) => {
    ...
    try {
      ...
      if (data && data.success && Array.isArray(data.incidents)) {
        ...
        if (newIncidentsToAdd.length > 0) {
          for (const inc of newIncidentsToAdd) {
  ...
  const handleResetDatabase = async () => {
    if (confirm('¿Desea restaurar los datos de simulación por defecto? Esto borrará sus cambios locales y los sincronizará en la nube.')) {
      // Clean Firestore
